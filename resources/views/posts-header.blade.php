        ...
        <div class="relative lg:inline-flex bg-gray-200 rounded-xl ">
            <x-drop-down>
                ...
                @foreach ($categories as $category)
                    <x-dropdown-item href="/categories/{{ $category->slug }}" class="{{ isset($currentcategory) && $currentcategory->is($category) ? 'bg-blue-500 text-white' : '' }}">{{ ucwords($category->name) }}</x-dropdown-item>
                @endforeach
            </x-dropdown-item>
        </div>
